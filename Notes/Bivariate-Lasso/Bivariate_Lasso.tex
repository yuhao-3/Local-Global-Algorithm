...
\begin{array}{rl}
	...
	+ 
	|\Sigma_2|
	[1,-1]^T (\frac{\int_0^\infty\int_0^\infty x\otimes\phi_2(x;\mu_2,\Sigma)d\textbf{x}}{\phi_2(A^*\mu_2,\Sigma_2^{-1}))}\\
	& \qquad
	+ [-1,1]^T   \frac{\int_0^\infty\int_0^\infty x\otimes\phi_2(x;\mu_3,\Sigma)d\textbf{x}}{\phi_2(A^*\mu_3,\Sigma_2^{-1})})]
	\\
	& \qquad
	=  Z^{-1}[|\Sigma_1|(\frac{E[A]\int_0^\infty\int_0^\infty x\otimes\phi_2(x;\mu_1,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_1,\Sigma_1^{-1}))}
	-  \frac{E[D]\int_0^\infty\int_0^\infty x\otimes\phi_2(x;\mu_4,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_4,\Sigma_1^{-1}))})\\
	& \qquad
	+ 
	|\Sigma_2|(
	[1,-1]^T \frac{E[B]\int_0^\infty\int_0^\infty x\otimes\phi_2(x;\mu_2,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_2,\Sigma_2^{-1}))}
...
\noindent where $\mu_1 = A^{-1}(b-c\textbf{1})^T$, $\mu_2 = A^{*-1}(b_1-c,-b_2-c)^T, \mu_3 = A^{*-1}(-b_1-c,b_2-c)^T \mu_4 = A^{-1}(-b-c \textbf{1}^T)^T $ and $\Sigma_1 = A^{-1}$, $\Sigma_2 = A^{*-1}$ $A^* = A \otimes 	\begin{bmatrix}
	...
	-1 & 1\\
\end{bmatrix}$.
\noindent $\textbf{A}\sim MTN_+(\mu_1,\Sigma_1)$, $B\sim MTN_+(\mu_2,\Sigma_)$, $C\sim MTN_+(\mu_3,\Sigma_2)$, $D\sim MTN_+(\mu_4,\Sigma_1)$ is denotes the multivariate positively truncated normal distribution.
...
\begin{array}{rl}
	...
	\\
	&
	=  Z^{-1}[|\Sigma_1|(\frac{E[A]\Phi_2(\frac{\mu_1}{\Sigma_1},\rho_1)}{\phi_2(A\mu_1,\Sigma_1^{-1}))}
	-  \frac{E[D]\Phi_2(\frac{\mu_4}{\Sigma_1},\rho_1)}{\phi_2(A\mu_4,\Sigma_1^{-1}))})\\
	& \qquad
	+ 
	|\Sigma_2|(
	\begin{bmatrix}
	1 & -1\\
	-1 & 1\\
\end{bmatrix} \otimes \frac{E[B]\Phi_2(\frac{\mu_2}{\Sigma_2},\rho_2)}{\phi_2(A^*\mu_2,\Sigma_2^{-1}))}

	+\begin{bmatrix}
		1 & -1\\
		-1 & 1\\
	\end{bmatrix} \otimes   \frac{E[C]\Phi_2(\frac{\mu_3}{\Sigma_2},\rho_2)}{\phi_2(A^*\mu_3,\Sigma_2^{-1})})]
	\\
