...
## Test effectiveness of Local-Global Algorithm 1
# Testing
```{r}
...
## Test effectiveness of Local-Global Algorithm 2
...
source(here("code","bivariate_local_global.R"))
...
{
  time_LG2_new <- system.time({
    res_LG2_new = bivariate_local_global(vy, mX, lambda, res_mfvb$vmu_til,res_mfvb$mSigma_til,res_mfvb$a_til,res_mfvb$b_til,damp = 0.5) 
  })
...
# Algorithm 3
```{r, eval=FALSE}
source(here("code","local_global_algorithms3.R"))

if(file.exists(paste("res_LG3_new",dataset_name,".Rdata",sep="")))
{
  load(paste("res_LG3_new",dataset_name,".Rdata",sep=""))
}else
{
  time_LG3_new <- system.time({
    res_LG3_new = local_global_algorithm_3(vy, mX, lambda, res_mfvb$vmu_til,res_mfvb$mSigma_til,res_mfvb$a_til,0.5, res_mfvb$b_til,res_bem$vbeta) 
  })
  save.image(paste("res_LG3_new",dataset_name,".Rdata",sep=""))
}
...
plot_densities <- function(ldens, cols, exclude, gold, threshold = 0.1) {
  vacc <- c()
  
  xlim <- c()
  ylim <- c()
  for (k in 2:length(ldens)) {
    if (!(k %in% exclude)) {
...
pdf(here("figures",paste("lasso_densities_",dataset_name,".pdf",sep="")), width= 20, height=10)
...
for (j in 1:p) {
  ...
  ldens[[4]] <- list(x=xg, y = dlasso(xg,va[j],vb[j],cc))
  ldens[[5]] <- list(x=xg, y = dnorm(xg,vmu_LG[j], vsigma_LG[j]))
  ...
  mAcc[,j] <- vacc
  title(sub = paste("Predictor", j), cex.sub = 2)
  ...
  methods <- c("BEM+SE","MCMC", "VB", "LG_Local","LG_Global")
    legend("topright",
           legend=methods[-1],
           text.col=cols[2:5],
           border = FALSE,
...
# Draw R code for lasso distribution
```{r}
# Load necessary libraries
library(ggplot2)

# Generate the PDF for different parameter settings
x <- seq(-10, 10, length.out = 1000)

# Set the parameters for the ExampleDistribution (a, b)
params <- list(c(1,0,0.5), c(1, 5, 0.5), c(1, -5, 0.5), c(5,0,0.5), c(1,0,5),c(5,5,5))

pdf_data <- data.frame()

# Calculate the PDF for each parameter setting
for (param in params) {
  a <- param[1]
  b <- param[2]
  c <- param[3]
  ...
  temp_data <- data.frame(x = x, y = y, a = a, b = b,c=c)
  pdf_data <- rbind(pdf_data, temp_data)
}


# Combine the a, b, and c parameters into a single label
pdf_data$label <- paste("a =", pdf_data$a, ", b =", pdf_data$b, ", c =", pdf_data$c)


# Create the plot with a white background
p <- ggplot(pdf_data, aes(x = x, y = y, color = interaction(a, b, c))) +
  geom_line() +
  scale_color_discrete(name = "Parameters\n(a, b, c)") +
  labs(x = "X", y = "Density", title = "Univariate Lasso Distribution PDF for Different Parameter Settings") +
...
ggsave("figures/lasso_distribution.png", plot = p, width = 14, height = 6) # width
