...
for (j in 1:p) {
  ...
  # ldens[[7]] <- list(x=xg, y = dmlasso(xg,vA[j],vb[j],cc))
  ldens[[7]] <- list(x=xg, y = dnorm(xg,vmu_LG2[j], vsigma_LG2[j]))
  
  vacc <- plot_densities(ldens, cols, exclude, gold=2)
  
  mAcc[,j] <- vacc
  
  methods <- c("BEM+SE","MCMC", "VB", "VB-PHC","LG_Local","LG_Global","LG2_Global")
