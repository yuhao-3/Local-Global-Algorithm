...
fun <- function(x)
{
  ...
  sigma2 = 1/A[2,2]
  Z <- zmlasso(A,b,c)
  z1 = zlasso(A[1,1],b[1],c)
  z2 = zlasso(A[2,2],b[2],c)
  ...
  print(z2)
  print(k)
  print(sqrt(sigma2)*(exp(log_partI)+exp(log_partII)))

  
  # return(k*sqrt(sigma2) * (pnorm(mu1/sqrt(sigma2)/dnorm(mu1/sqrt(sigma2))+ pnorm(mu2/sqrt(sigma2))/dnorm(mu2/sqrt(sigma2)))))
  return(exp(log_mpdf)) 
  
}

integrate(fun,lower = -Inf, upper = Inf)
```
