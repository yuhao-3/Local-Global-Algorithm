...
params = bayesian_lasso_mfvb(y,X,lambda = 1,sigma2_hat = 1)
params
```
...
{
  # Log trick
  PartI = exp(pnorm(mu1/sigma,log.p = TRUE)- dnorm(mu1/sigma,log = TRUE))
  PartII = exp(pnorm(mu2/sigma,log.p = TRUE)- dnorm(mu2/sigma,log = TRUE))
  Z = sigma*(PartI + PartII)
  return(Z)
}
...
calculate_multi_lasso_param <- function(mA,vb,cc)
{
  ...
  PartI = exp(log(pmvnorm(mean = mu1, sigma =Sigma))-log(dmvnorm(as.vector(A%*%mu1),sigma = Sigma)))
  PartII = exp(log(pmvnorm(mu2, sigma =Sigma))-log(dmvnorm(as.vector(A%*%mu2),sigma = Sigma)))
  PartIII = exp(log(pmvnorm(mu3, sigma = Sigma))-log(dmvnorm(as.vector(A%*%mu3),sigma = Sigma)))
  PartIV = exp(log(pmvnorm(mu4, sigma=Sigma))-log(dmvnorm(as.vector(A%*%mu4),sigma = Sigma)))
  
  # Calculation
  Z = multi_norm_const_lasso(PartI, PartII, PartIII,PartIV)
  # mu = mean_multi_lasso(PartI, PartII, PartIII,PartIV, Z)
  print(Z)
  # var = var_multi_lasso(PartI, PartII, PartIII,PartIV, Z)
  return(list(mu = mu,var = var, Z = Z))
}

multi_norm_const_lasso <- function(PartI, PartII, PartIII,PartIV)
{
  # Log trick
...
# Calculate expectation of lasso distribution
mean_multi_lasso <- function(PartI, PartII, PartIII, PartIV, Z)
{
...
var_multi_lassol = function(PartI, PartII, PartIII, PartIV, Z)
{
...
result_LG = local_global_algorithm_1(y,X,lambda,params)
result_LG
```
...
cc = result_LG$c
vZ = result_LG$Z
global_mu = result_LG$mu
global_Sigma = result_LG$Sigma
...
for (j in 1:p)
{
   # returns the density of MCMC samples
	 mcmc = density(Samples$beta[,j][-1], from = -1.5, to = 1.5)
	 ...
	 c = cc
	 Z = vZ[j]
	 lasso = curve(exp(-0.5*a*x^2+b*x-c*abs(x))/Z,add = TRUE,from = - 1.5, to = 1.5,col="green")
	 

	 # draw global parameter distribution
	 global = curve(dnorm(x, mean = global_mu[j],sd = sqrt(global_Sigma[j,j])), add = TRUE,from = -1.5, to = 1.5,col="red")
	 
   legend("topleft", legend=c("MCMC","Local","Global"),
       col=c("blue", "green", "red"), lty=1:2, cex=0.4,
       title="Density types", text.font=4, bg='lightblue',plot = TRUE)
   
}
# 
save.image(here("results",paste("densityresults.Rdata",sep="")))
```
