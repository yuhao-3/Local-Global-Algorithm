...
\begin{equation}
	\label{LocalGlobalVI}
	\log(\mathcal{D},\theta_1) = \mathbb{E}_{q(\theta_{2}|\theta_1)}\left[\log(\frac{p(\mathcal{D},\theta_1,\theta_{2})}{q(\theta_{2}|\theta_1)})\right] + KL(q(\theta_{2}|\theta_1),p(\theta_{2}|\mathcal{D},\theta_1))
\end{equation}
...
\begin{equation}
	\log(\mathcal{D},\theta_1) \geq \mathbb{E}_{q(\theta_{2}|\theta_1)}\left[\log(\frac{p(\mathcal{D},\theta_1,\theta_{2})}{q(\theta_{2}|\theta_1)})\right]
\end{equation}
...
In Bayesian Lasso: $\theta = (\beta,\sigma^2)$, however, the update for $\sigma$ will not be discussed while we will only discuss an approach for updating $\beta$, assuming $q(\beta) \sim N(\mu,\Sigma)$. Thus,
...
\begin{equation}
	p(\mathbf{x}) = Z^{-1}\exp\left(-\frac{1}{2}\mathbf{x}^TA\mathbf{x}+b^T\mathbf{x}-c||\mathbf{x}||_1\right)
\end{equation}
...
\begin{array}{rl}
	...
	&
	=|\Sigma_1| \left(\frac{\int_0^\infty\int_0^\infty \phi_2(x;\mu_1,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_1,\Sigma_1^{-1})} 
	+ \frac{\int_0^\infty\int_0^\infty \phi_2(x;\mu_4,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_4,\Sigma_1^{-1})}\right) \\
	& \qquad	
	+ |\Sigma_2| \left(\frac{\int_0^\infty\int_0^\infty \phi_2(x;\mu_2,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_2,\Sigma_2^{-1})} + \frac{\int_0^\infty\int_0^\infty \phi_2(x;\mu_3,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_3,\Sigma_2^{-1})} \right)
...
\begin{array}{rl}
	...
	& \qquad
	=  Z^{-1}[|\Sigma_1|\left(\frac{\int_0^\infty\int_0^\infty x\odot\phi_2(x;\mu_1,\Sigma)d\textbf{x}}{\phi_2(A\mu_1,\Sigma_1^{-1})}
	-  \frac{\int_0^\infty\int_0^\infty x\odot\phi_2(x;\mu_4,\Sigma)d\textbf{x}}{\phi_2(A\mu_4,\Sigma_1^{-1}))}\right)\\
	& \qquad
	+ |\Sigma_2|
	\left([1,-1]^T\frac{\int_0^\infty\int_0^\infty x\odot\phi_2(x;\mu_2,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_2,\Sigma_2^{-1})}
	+ [-1,1]^T   \frac{\int_0^\infty\int_0^\infty x\odot\phi_2(x;\mu_3,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_3,\Sigma_2^{-1})})\right)
	\\
	& \qquad
...
\begin{array}{rl}
	...
	\\
	&
	=  Z^{-1}[|\Sigma_1|\left(\frac{\mathbb{E}[\textbf{A}\textbf{A}^T]\int_0^\infty\int_0^\infty \phi_2(x;\mu_1,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_1,\Sigma_1^{-1})}
	+  \frac{\mathbb{E}[DD^T]\int_0^\infty\int_0^\infty \phi_2(x;\mu_4,\Sigma_1)d\textbf{x}}{\phi_2(A\mu_4,\Sigma_1^{-1})}\right)\\
	& \qquad
	+ 
	|\Sigma_2|\left(
	\begin{bmatrix}
	...
	+\begin{bmatrix}
		1 & -1\\
		-1 & 1\\
	\end{bmatrix} \odot   \frac{\mathbb{E}[CC^T]\int_0^\infty\int_0^\infty \phi_2(x;\mu_3,\Sigma_2)d\textbf{x}}{\phi_2(A^*\mu_3,\Sigma_2^{-1})}\right)]
	\\
...
Continuing from \autoref{eq:MarLike}, it is evident to observe that\\ $p(\beta_j|\mathcal{D}) \propto p(\beta_j,\mathcal{D}) \sim Lasso\left(\frac{\tilde{a}}{\tilde{b}}(y - X_{-j}s), \frac{\tilde{a}}{2\tilde{b}}(X_j^TX_j+X_j^TX_{-j}t) , \frac{\lambda \Gamma(\tilde{a}+1/2)}{\Gamma(\tilde{a})\sqrt{\tilde{b}  }}\right) $.
...
For the purpose of enabling $\beta_{-\sI}$ to be symmetric for preventing generating a non-symmetric, the log-likelihood of index pair $\sI$ can be remodified as the following:
\begin{equation}
	\begin{array}{rl} 
