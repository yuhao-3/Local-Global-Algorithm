...
the parameter of interest $\theta$ can be divided up into two parts $\theta_1$ and $\theta_{2}$. The marginal log likelihood of $\theta_1$ can be divided up into ELBO part and KL divergence part as the following:
\begin{equation}
...
The derivation for $\tilde{\mu}$ has been shown below using the property of the conditional expectation:

We know that $q(\theta_2|\theta_1)$ and $q(\theta_1)$ are Gaussian. Hence, their joint distribution will also be Gaussian. The marginal 
...
All we need to do is determine $\widetilde{\mu}_{2}$, $\widetilde{\Sigma}_{22}$
...
\noindent Hence, $q^*(\theta) = N(\widetilde{\mu},\widetilde{\Sigma})$ at the time when updating variable $j$ is:
\begin{equation}
	\widetilde{\mu} =
	\left[ \begin{array}{c}
		\mu_j^* \\
...
\begin{itemize}
	\item Our target: $p(\theta|\mathcal{D})$ = $p(\beta|\mathcal{D})$, where a normal distribution is used to approximate, with global parameters $\tilde{\mu},\tilde{\Sigma}$. The initial estimate of $\tilde{\mu}$ and $\Sigma$ has been obtained via MFVB. Define $\theta = (\theta_j,\theta_{-j})$, separated by a target variable and rest of the variables. $\mu_{j}^*, \Sigma_{jj}^*$ as local parameter
	
	
	\item We can derive and calculate the mean of lasso distribution and variance of lasso distribution $a,b,c$: $q^*(\theta_1) \approx N(\mu_1^*,\Sigma_{11}^*)$
	\item We can use the Expression $q^*(\theta) = q(\theta_{2}|\theta_1)\phi(\theta_1;\mu_1^*,\Sigma_{11}^*)$ to update $q(\theta)$
