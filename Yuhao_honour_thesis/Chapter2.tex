...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $\tilde{\mu} = Q^{-1}X^Ty$ \Comment{Update $\beta$}
		\State $\tilde{\Sigma} = Q^{-1}\mathbb{E}_{q}[\sigma^2] $ \Comment{Update $\beta$}
		\State $\tilde{a} = \frac{n+p}{2}$ \Comment{Update $\beta$}
		\State $\tilde{b} = \frac{E_{q}||y-X\beta||^2 + \lambda^2 \mathbb{E}_{q}[\beta^TA\beta]}{2}$\Comment{Update $\beta$}
		\EndWhile  
