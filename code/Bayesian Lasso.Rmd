...
params = bayesian_lasso_mfvb(y,X,lambda = 10,sigma2_hat = 1)
params
```
...
Exact case
$\mu_1 = -1$, $\mu_2 = -5, sigma2 = 1$
$$
Z = 1 * [\frac{pnorm(-1)}{dnorm(-1)} +\frac{pnorm(-5)}{dnorm(-5)}] = 0.8484876
$$
$$
E[X] = \frac{1}{Z} * [\frac{pnorm(-1)}{dnorm(-1)}E[A] +\frac{pnorm(-5)}{dnorm(-5)}E[B]]
$$

$$
E[A] = -1 + \frac{dnorm(-1)}{pnorm(-1)} = 0.52513\space53\\
E[B] = -5 + \frac{dnorm(-5)}{pnorm(-5)} = 0.186504\\

E[X] = 1/0.8484876 *(pnorm(-1)/dnorm(-1)*0.5251353 - pnorm(-5)/dnorm(-5)*0.186504)= 0.3634243
$$

$$
E[A^2] = 1*(1+xsi_2(-1)) + 0.52513 53^2 = 0.4748647\\
E[B^2] = 1*(1+xsi_2(-5)) + 0.186504^2 = 0.06748018 \\
xsi2(-1) = 1*xsi_1(-1) - (xsi_1(-1))^2 = -0.8009023\\
xsi2(-5) = 5*xsi_1(-5) - (xsi_1(-5))^2 = -0.9673036\\

E[X^2] = 1/0.8484876 *(pnorm(-1)/dnorm(-1)*0.4748647 + pnorm(-5)/dnorm(-5)*0.06748018)= 0.3822917
...
local_global_algorithm_1 <- function(vy, mX, beta, lambda, params)
{
  ## Initialization
  MAXITER <- 500
  TOL <- 1.0E-8
  ...
  # Initial value for a,b,c, Local parameters
  va = rep(0,p)
  vb = rep(0,p)
  vc = rep(0,p)

  # Stop if global parameter converge
  vtheta <- c(vmu_adjust, mSigma_adjust)
  ...
  for (ITER in 1:MAXITER)
  {
    ...
    for (j in 1:p)
    {
      ## Get conditional distribution update: q(beta_j | D) for each beta_j
      
      ## vmu_adjust, mSigma_adjust
      vs = vmu_til[-j] - mSigma_til[-j,j] %*% solve(mSigma_til)[j,j] %*% vmu_til[j]
      mt = mSigma_til[-j,j] %*% solve(mSigma_til)[j,j]
      va[j] = a_tilde/b_tilde*(t(X[,j]) %*% X[,j] + t(X[,j])%*%X[,-j]%*%mt)
      vb[j] = a_tilde/b_tilde*t(X[,j])%*%(vy-X[,-j]%*%vs)
      vc[j] = lambda * gamma(a_til+0.5)/(gamma(a_til)*sqrt(b))
      local_mean = calculate_lasso_param(a,b,c)$mu
      local_var = calculate_lasso_param(a,b,c)$var
      
      # Current Conditional Param
      vcond_mean = vmu_til[-j]+mSigma_til[-j,j]%*%solve(mSigma_til)()
      mcond_covariance = 
    
      ##  Cond param + local update = Global Update    
      vmu_adjust= cond mean + local mean
      mSigma_adjust = cond covariance + local covariance     
    }
...
```{r}
local_global_algorithm_2 <- function(vy, mX, lambda, sigma2_hat)
{
