...
local_global_algorithm_1 <- function(vy, mX, lambda, beta, params)
{
  ## Initialization
  MAXITER <- 500
  TOL <- 1.0E-8
  ...
  for (ITER in 1:MAXITER)
  {
    ## Problem, some matrix can be 1*1 or p*p, can't be multipied
    ...
    for (j in 1:p)
    {
      ...
      ## Local Update
      vs = vmu_adjust[-j] - mSigma_adjust[-j,j] %*% solve(mSigma_adjust)[j,j] %*% vmu_adjust[j]
      mt = mSigma_adjust[-j,j] %*% solve(mSigma_adjust)[j,j]
      ...
      vmu_adjust[-j] = vmu_adjust[-j] + mSigma_adjust[-j,j] %*% solve(mSigma_adjust[-j,-j])%*%(vlocal_mean[j] - vmu_adjust[j])
      
      ##  Update Covariance
      mSigna_minus_jj_inv = solve(mSigma_adjust[-j,-j])
      mSigma_adjust[j,j] = mlocal_var
      mSigma_adjust[j,-j] = mlocal_var %*% mSigna_minus_jj_inv %*% mSigma_adjust[j,-j]
      mSigma_adjust[-j,j] = t(mSigma_adjust[j,-j])
      mSigma_adjust[-j,-j] = mSigma_adjust[j,j] + mSigma_adjust[j,-j] %*% mSigna_minus_jj_inv
      (mlocal_var - mSigma_adjust[-j,-j]) %*% mSigna_minus_jj_inv %*% mSigma[-j,j]
    }
  ...
  return(list(vmu_adjust,mSigma_adjust))
}

local_global_algorithm_1(y,X,lambda = 10,params)
